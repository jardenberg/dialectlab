const { APP_VERSION, ASSET_VERSION, BASE_URL, HERO_LOCALE_OPTIONS, getAlternateLinks, getCanonicalUrl, getLocalePath, shared } = require('./locales');
...
function renderPage(locale, bundle) {
  ...
  const footerVersion = `v${APP_VERSION} (${ASSET_VERSION})`;
  const localeLinks = HERO_LOCALE_OPTIONS.map(
    (option) =>
      `<a href="${escapeHtml(getLocalePath(option.locale))}" class="locale-link ${
        option.locale === locale ? 'is-active' : ''
      }" lang="${escapeHtml(option.locale)}"${option.locale === locale ? ' aria-current="page"' : ''}>${escapeHtml(
        option.label,
      )}</a>`,
  ).join('');
    ...
    <div class="page-shell">
      <header class="hero-card">
        <div class="hero-top">
          ...
          <nav class="locale-switcher" aria-label="${escapeHtml(copy.languagePickerLabel)}">
            ${localeLinks}
          </nav>
        </div>
      ...
      <main class="content-grid">
        ...
        <section class="panel panel-output panel-dark">
          ...
          <div class="audio-wrap">
            ...
            <div class="action-row">
              ...
              <a id="download-link" class="secondary-button secondary-link hidden" href="#" download>${escapeHtml(copy.downloadAudio)}</a>
            </div>
          </div>
          ...
          <div class="settings-block">
            ...
            <div class="slider-card">
              <label class="slider-label" for="tempo-slider">
                ...
                <strong id="tempo-value">0.95x</strong>
              </label>
        ...
        <section class="panel panel-transformed panel-accent">
      ...
      <footer class="panel site-footer">
        <div class="panel-label">${escapeHtml(copy.footerLabel)}</div>
        <h2>${escapeHtml(copy.footerTitle)}</h2>
        <p class="footer-copy">
          ${escapeHtml(copy.footerCopy)}
          <a class="footer-link" href="mailto:joakim@jardenberg.com">joakim@jardenberg.com</a>
        </p>
        <div class="tag-row tag-row-footer">
          ${copy.tags.map((tag) => `<span>${escapeHtml(tag)}</span>`).join('')}
        </div>
        <div class="footer-meta">${escapeHtml(copy.footerVersionLabel)} <strong>${escapeHtml(footerVersion)}</strong></div>
      </footer>
    </div>
