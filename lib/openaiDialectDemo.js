...
async function rewriteToSkanska(openai, transcript, strength) {
  const targetDialect = getTargetDialect();
  const response = await openai.responses.create({
    model: process.env.DIALECTLAB_TEXT_MODEL || 'gpt-4.1-mini',
    instructions: [
      ...
      'Do not translate away from Swedish.',
      'Keep both outputs concise and conversational.',
      ...
      `tts_text: a more aggressive pronunciation script for TTS. It may use non-standard spellings, contractions, apostrophes, commas, ellipses, and stretched vowels if that helps the engine sound more ${targetDialect}.`,
      'Do not use IPA.',
  ...
  return parseRewritePayload(response.output_text?.trim() || '', transcript);
}
...
async function synthesizeWithElevenLabs(text, speechSpeed) {
...
async function synthesizeSpeech(openai, text, voice, strength, speechSpeed) {
  ...
  if (backend === 'elevenlabs_tts') {
    return synthesizeWithElevenLabs(text, speechSpeed);
  }
...
async function transformSpeechToDialect(req) {
  ...
  const transcript = await transcribeSpeech(openai, req.file);
  if (!transcript) {
    throw createHttpError(422, 'No speech was detected in the uploaded audio.');
  }

  const rewritten = await rewriteToSkanska(openai, transcript, strength);
  ...
  const resolvedVoice = audioBackend === 'elevenlabs_tts' ? getElevenLabsVoiceId() : voice;
  const speakerMode =
  ...
  return {
    ...
    meta: {
      ...
      rewriteModel: process.env.DIALECTLAB_TEXT_MODEL || 'gpt-4.1-mini',
      ttsModel:
      ...
      warnings:
        audioBackend === 'elevenlabs_tts'
          ? [
              'Audio is rendered by ElevenLabs using the configured Skanska voice. OpenAI is still used for transcription and rewrite control.',
            ]
          : speakerMode === 'builtin'
          ? [
...
function getPublicConfig() {
  const audioBackend = getAudioBackend();
  const defaultVoice = audioBackend === 'elevenlabs_tts' ? 'skanska-elevenlabs' : getConfiguredVoice('');
  ...
  return {
    ...
    audioBackend,
    defaultVoice,
    availableVoices: audioBackend === 'elevenlabs_tts' ? [defaultVoice] : BUILTIN_VOICES,
